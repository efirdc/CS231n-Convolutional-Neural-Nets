...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hidden1, hidden1_cache = affine_relu_forward(X, W1, b1)
        hidden2, hidden2_cache = affine_forward(hidden1, W2, b2)
        scores = hidden2
        ...
        loss = softmax + 0.5 * self.reg * (np.sum(W1 * W1) + np.sum(W2 * W2))
        dW1 = self.reg * W1
        dW2 = self.reg * W2

        dhidden2, dW2_hidden2, db2 = affine_backward(dsoftmax, hidden2_cache)
        dW2 += dW2_hidden2

        dhidden1, dW1_hidden1, db1 = affine_relu_backward(dhidden2, hidden1_cache)
        dW1 += dW1_hidden1
